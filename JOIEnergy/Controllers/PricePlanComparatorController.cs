...
namespace JOIEnergy.Controllers
{
    ...
    public class PricePlanComparatorController : Controller
    {
        ...
        [HttpGet("PeriodConsumption/{smartMeterId}")]
        public ObjectResult CalculatedPeriodCostForEachPricePlan(string smartMeterId)
        {
            var PeriodConsumptionForPricePlans = _pricePlanService.GetPeriodConsumptionCostOfElectricityReadingsForEachPricePlan(smartMeterId);

            if (!PeriodConsumptionForPricePlans.Any())
            {
                return new NotFoundObjectResult(string.Format("Smart Meter ID ({0}) do not have a price plan", smartMeterId));
            }

            var Conputions = PeriodConsumptionForPricePlans.OrderBy(pricePlanComparison => pricePlanComparison.Value);

            return new ObjectResult(Conputions);
        }
    }
}
